...
const FilterSideBar = ({filterPhone, setFilterPhone, filters, setFilters}) => {
  ...
  return (
    <div className={` ${filterPhone? "max-sm:fixed" : "max-sm:hidden"} max-sm:inset-0 z-100 max-sm:h-screen max-sm:overflow-scroll bg-white rounded-lg shadow-sm border border-gray-200 h-fit sticky top-24 md:min-w-[300px]`}>
      <div className='p-4 border-b border-gray-200'>
        <div className='flex items-center justify-between'>
            ...
            <button onClick={()=> setFilterPhone(false)} className="border sm:hidden text-sm text-gray-700 px-3 py-1 rounded">Apply</button>
        </div>
      </div>
