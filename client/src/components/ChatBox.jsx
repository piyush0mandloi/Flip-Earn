        ...
        <div className='flex-1 overflow-y-auto p-4 space-y-4 bg-gray-100'>
          {isLoading ? (
            ...
            <div className='flex items-center justify-center h-full'>
              ...
              <p className='tex-gray-500 mb-2'>No messages yet</p>
            ...
            messages.map((message)=>(
              <div key={message.id} className={`flex ${message.sender_id===user.id ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[70%] rounded-lg p-3 pb-1 ${message.sender_id===user.id? "bg-indigo-600 text-white" : "bg-white border border-gray-200 text-gray-800"}`}>
                  <p className='text-sm break-word whitespace-pre-wrap'>{message.message}</p>
...
{chat?.listing?.status === "active" ? (
  <form onSubmit={handleSendMessage} className='p-4 bg-white border-t border-gray-200 rounde-b-lg'>
