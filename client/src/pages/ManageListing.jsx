...
const ManageListing = () => {
  ...
  const platforms = ['youtube', 'instagram', 'ticktok','facebook','twitter','linkedin','pinterest','snapchat','twitch','discord']
  const niches = ['lifestyle','fitness','food','travel','tech','gaming','fashion','beuty','business','education','entertainment','music','art','sports','health','finance','other']
      ...
      <form onSubmit={handleSubmit} className='space-y-8'>
        ...
        <Section title='Account Metrics'>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
            ...
            <InputField label='Monthly Views/Impressions' type='number' min={0} value={formData.monthly_views} placeholder='10000' onChange={(v)=> handleInputChange('monthly_views',v)}/>
          </div>
          ...
          <div className='space-y-3'>
            <CheckBoxField label="Account is verified on the platform" checked={formData.verified} onChange={(v)=> handleInputChange('monetized', v)}/>
            <CheckBoxField label="Account is verified on the platform" checked={formData.verified} onChange={(v)=> handleInputChange('monetized', v)}/>
          </div>
        </Section>
        {/* Pricing */}
        ...
        <Section title="Screenshots & Proofs">
          <div className='border-2 border-dashed border-gray-300 rounded-lg p-6 text-center'>
            <input type="file" id='images' multiple accept='images/*' onChange={handleImageUpload} className='hidden'/>
        ...
        <div className='flex justify-end gap-3  text-sm'>
          <button type="submit" onClick={()=> navigate(-1)} className='px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors'>
            Cancel
          </button>
...
const TextareaField=({label, value, onChange, required=false})=>(
  <div> 
  <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
    <textarea rows={5} value={value} onChange={(e)=>onChange(e.target.checked)} className='w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300' required={required} />
  </div>
)
